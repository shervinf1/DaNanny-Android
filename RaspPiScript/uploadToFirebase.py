...
cred = credentials.Certificate(
    '/home/pi/Desktop/energy-core-firebase-adminsdk-2lnli-4d6dc8dbe9.json')
...
def create_reading_firestore():
    ...
    if(len(sensorData) == 14):
        reading = (dateTime, sensorData[0], sensorData[1], sensorData[2], sensorData[3], sensorData[4], sensorData[5], sensorData[6],
                   sensorData[7], sensorData[8], sensorData[9], sensorData[10], sensorData[11], sensorData[12], sensorData[13])
        print(reading)
        VBATT = sensorData[0]
        TEMP = sensorData[1]
        FANID = sensorData[2]
        ...
        gen_ref.set({
            u'date': dateTime,
            u'current': float(CURRGEN),
            u'voltage': float(VGEN),
            u'watts': float(WGEN),
            u'userID': userId,
            u'sourceID': sourceId
        })
        ...
        device1_ref.set({
            u'date': dateTime,
            u'current': float(CURRFAN),
            u'voltage': float(VFAN),
            u'watts': float(WFAN),
            u'userID': userId,
            u'deviceID': ledId
        })
        ...
        temp_ref.set({
            u'date': dateTime,
            u'temperature': float(TEMP),
            u'userID': userId
        })

        batt_ref.set({
            u'date': dateTime,
            u'batteryStatus': float(VBATT),
            u'userID': userId
        })
